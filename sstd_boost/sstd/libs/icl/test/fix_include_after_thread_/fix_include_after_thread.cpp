...
//Problem: If <sstd/boost/thread.hpp> is included before this
...
// #include <sstd/boost/thread.hpp> MEMO: The problem occured when using thread.hpp
#include <sstd/boost/bind.hpp>    // but is also triggered from bind.hpp alone
...
template <class Type> struct some_predicate
{
    ...
    //  value = ( some_attribute<Type>::value  < 0)
                ...
                //IF <sstd/boost/thread.hpp> is included before
        );
};
